  ...
  <view ref="themeCount" class="themeCount" v-if="!isDeleted">
    ...
    <view class="themeItem" @click="backgroundClick">
      ...
      <view class="themeItem__content" @click.stop="" @click="contentClick">
        ...
        <view v-else><!--if="imagesList.length == 2"-->
          <view class="themeItem__content__imgtwo-carousel" v-if="imagesList.length >= 5">
            <image
              :class="[isBigPicture ? 'themeItem__content__imgtwo__item_big' : 'themeItem__content__imgtwo__item']"
              v-for="(image, index) in imagesList.slice(0,5)"
              :key="index"
              :mode="modeVal"
              :src="image.thumbUrl"
              @click="previewPicture(index)"
          ...
          <view>
            <view class="themeItem__content__imgmore">
              <view
                :class="[isBigPicture ? 'themeItem__content__imgmore__item-big':'themeItem__content__imgmore__item']"
                v-for="(image, index) in imagesList.slice(2, 10)"
                :key="index"
                :style="{ position: index === 8 ? 'relative' : '' }"
              >
                <image
                  :class="[isBigPicture ? 'imgmore-image-big':'imgmore-image']"
                  :mode="modeVal"
                  :src="image.thumbUrl"
                  @click="previewPicture(index)"
                  @click.stop=""
...
export default {
  ...
  props: {
    ...
    // 是否启用轮播模式，列表大图轮播展示 【isBigPicture启用状态生效】
    isPictureCarousel: {
      type: Boolean,
      default: false,
    },
