              ...
              >
                <view class="attachment-name">
                  <qui-icon
                    ...
                    :name="fileItem.name ? `icon-${fileItem.format}` : `icon-resources`"
                    size="22"
                  ></qui-icon>
...
export default {
  ...
  data() {
    return {
      ...
      loginStatus: 0, // 附件上传 0 扩展信息 1其他上传
      header: {}, // 图片请求头部
    };
  },
  computed: {
  ...
  methods: {
    ...
    handleDeleteOk() {
      ...
      if (this.deleteType === 0) {
        ...
        fieldList.forEach((item, index) => {
          if (this.deleteId === item.id) {
            fieldList.splice(index, 1);
            this.$refs.uploadFiles.deleteItem(item.id);
          }
        });
      }
    },
    ...
    uploadFileBtn(eIndex) {
      this.$refs.uploadFiles.uploadBtn();
      this.$refs.uploadFiles.setIndex(eIndex);
    },
    // 附件列表改变
    fileChange(e) {
      const fileList = e;
      this.dataList.forEach((item, index) => {
        if (item.type === 5) {
          this.dataList[index].fields_detail = [];
        }
      });
      fileList.forEach(v => {
        this.dataList[v.index].fields_detail.push({
          id: v.id,
          name: v.attributes.fileName,
        ...
        if (item.type === 0 || item.type === 1) {
          ...
          if (item.fields_detail.length > 0) {
            ...
            item.fields_detail.forEach(imgItem => {
              fileList.push({
                id: imgItem.id,
                name: imgItem.attributes.fileName,
    ...
    handleRegister() {
      ...
      for (let i = 0; i < this.dataList.length; i += 1) {
        if (
          this.dataList[i].required === 1 &&
          (reg.test(this.dataList[i].fields_detail) || this.dataList[i].fields_detail.length < 1)
        ) {
          this.$refs.toast.show({ message: `${this.dataList[i].name}不能为空` });
          isNullData.push(0);
          return;
        }
        isNullData.push(1);
